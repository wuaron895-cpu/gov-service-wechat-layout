...
> 把任意民生政务号（社保/医保/公积金/税务/民政/公安政务等）的公众号推文，按 6 类内容套模板排版，换主题色、填占位即可发布。
...
一个 WorkBuddy skill，内置 6 类民生政务推文的排版模板 + 一套可换主题色的配色模型。编辑拿到定稿，选对类型、套模板、换色、填占位，就能产出可发布的排版。
...
本 skill 的 6 套模板**只用内联 `style=""`，无 `<style>`、无 `class`、无 CSS 变量、无脚本、无外部字体**（已校验）——正好是微信保留的子集。所以粘贴进后台，格式留得住。
...
每篇换一个主题色，全文贯穿；警示红、正确绿固定不换。

| 角色 | 色值 | 可换 | 用途 |
|---|---|---|---|
| 主题色 | `#ff6d4d`（占位） | ✅ | 标题、强调、卡片边、Q 标记、链接 |
| 警示红 | `#ff0000` | ❌ | ❎错误、注意、法律警告 |
| 正确绿 | `#2E9E5B` | ❌ | ✓ 正确结论 |

**候选主题色**：`#ff6d4d` 橙红 / `#be5960` 玫红 / `#ff8587` 粉红 / `#fa6729` 橙 / `#5f9cef` 蓝 / 通知公告用黑白。

**换色**：
- 脚本：`python scripts/apply_theme_color.py --input 模板.html --color 5f9cef`
- 手动：全文查找替换 `#ff6d4d`（警示红 `#ff0000`、正确绿 `#2E9E5B` 不要动）
- 秀米/135：导入后用"主题色"一键换
...
模板已去标识化。给新账号用：跑 `references/layout-sop.md` 的 5 步量该账号真实色值与组件，替换 `{{…}}` 占位即可。内容类型超出 6 类时，按 SOP 新增 `assets/template-*.md.html`。
...
- `shunde-social-135-publisher`（如有）：管**135 发布包**格式转换（T01/T02/T03 路由 + 校验）。两者互补，不重叠。
...
│   ├── component-library.md          组件+字号+6类骨架
│   └── layout-sop.md                 5步方法论（扩展用）
├── assets/                           6 套去标识化 HTML 模板
